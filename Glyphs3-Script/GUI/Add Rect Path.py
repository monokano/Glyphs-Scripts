...
import vanilla
import traceback
from AppKit import NSColor, NSPoint, NSAlert
...
Empty_the_glyph = Glyphs.localize({ 'en': u'Empty glyphs before adding', 'ja': u'矩形パス追加前にグリフを空にする', })
...
class MainWindow(object):
	def __init__(self):
		self.w = vanilla.FloatingWindow((310, 264+28), Add_Rect_Path, autosaveName="com.tama-san.MakeRectPath.mainwindow")
	...
	if (targetLayers is None) or (len(targetLayers)==0):
		...
		for thisLayer in targetLayers:
			...
			if (thisRightX - thisLeftX)>0:
				...
				if type==0:
					# White
					makeWhiteRect(Font, thisLayer, OffsetValue, thisLeftX, thisTopY, thisBottomY, thisRightX, lineWidth)
				else:
					# Black
					makeBlackRect(Font, thisLayer, OffsetValue, thisLeftX, thisTopY, thisBottomY, thisRightX)
...
def makeWhiteRect(Font, thisLayer, OffsetValue, thisLeftX, thisTopY, thisBottomY, thisRightX, lineWidth):
...
def makeBlackRect(Font, thisLayer, OffsetValue, thisLeftX, thisTopY, thisBottomY, thisRightX):
