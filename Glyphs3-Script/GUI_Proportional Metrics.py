...
localizedStrings = {
    'windowTitle': { 'en': u'Proportional Metrics', 'ja': u'プロポーショナルメトリクス', },
    'Left_Sidebearing': { 'en': u'Left Sidebearing:', 'ja': u'左 サイドベアリング:', },
    'Right_Sidebearing': { 'en': u'Right Sidebearing:', 'ja': u'右 サイドベアリング:', },
    'Top_Sidebearing': { 'en': u'Top Sidebearing:', 'ja': u'上 サイドベアリング:', },
    'Bottom_Sidebearing': { 'en': u'Bottom Sidebearing:', 'ja': u'下 サイドベアリング:', },
    'Set_Anchors': { 'en': u'Add Anchors', 'ja': u'アンカーを追加', },
    'Remove_Anchors': { 'en': u'Remove Anchors', 'ja': u'アンカーを削除', },
    'Move_Positions': { 'en': u'Move Anchor Positions', 'ja': u'アンカーを移動', },
    'Auto_Adjust': { 'en': u'Set', 'ja': u'設定する', },
    'Reset': { 'en': u'Unset', 'ja': u'解除', },
    'Limit': { 'en': u'Constrain', 'ja': u'SB内に制限', },
    'Reverse': { 'en': u'Reverse', 'ja': u'方向反転', },
    'Shift': { 'en': u'Shift:', 'ja': u'増減値:', },
    'L': { 'en': u'L', 'ja': u'左', },
    'R': { 'en': u'R', 'ja': u'右', },
    'T': { 'en': u'T', 'ja': u'上', },
    'B': { 'en': u'B', 'ja': u'下', },
    'Single_Mode': { 'en': u'Single Glyph Operation', 'ja': u'単グリフ操作', },
    'Reset_to_Defaults': { 'en': u'Reset to Defaults', 'ja': u'デフォルトに戻す', },
}

windowTitle = Glyphs.localize(localizedStrings['windowTitle'])
Left_Sidebearing = Glyphs.localize(localizedStrings['Left_Sidebearing'])
Right_Sidebearing = Glyphs.localize(localizedStrings['Right_Sidebearing'])
Top_Sidebearing = Glyphs.localize(localizedStrings['Top_Sidebearing'])
Bottom_Sidebearing = Glyphs.localize(localizedStrings['Bottom_Sidebearing'])
Set_Anchors = Glyphs.localize(localizedStrings['Set_Anchors'])
Remove_Anchors = Glyphs.localize(localizedStrings['Remove_Anchors'])
Move_Positions = Glyphs.localize(localizedStrings['Move_Positions'])
Auto_Adjust = Glyphs.localize(localizedStrings['Auto_Adjust'])
Reset = Glyphs.localize(localizedStrings['Reset'])
Limit = Glyphs.localize(localizedStrings['Limit'])
Reverse = Glyphs.localize(localizedStrings['Reverse'])
Shift = Glyphs.localize(localizedStrings['Shift'])
L = Glyphs.localize(localizedStrings['L'])
R = Glyphs.localize(localizedStrings['R'])
T = Glyphs.localize(localizedStrings['T'])
B = Glyphs.localize(localizedStrings['B'])
Single_Mode = Glyphs.localize(localizedStrings['Single_Mode'])
Reset_to_Defaults = Glyphs.localize(localizedStrings['Reset_to_Defaults'])
...
class Mainwindow(object):
	def __init__(self):
		...
		## window
		self.w = vanilla.FloatingWindow((232, 324+20), windowTitle, autosaveName="com.tama-san.ProportionalMetrics.mainwindow")
		self.w.bind("should close", self.savePreferences)
		
		# single mode
		self.w.checkSingleMode = vanilla.CheckBox((10+2, 5, 200, 20), Single_Mode, sizeStyle='small', value=False, callback=self.toggleSingleModeAction)
		
		## glyph name
		self.w.labelName = vanilla.TextBox((10, 5+20, -10, 20), "", alignment='center')
		...
		self.w.popupAction = vanilla.ActionButton((188, 6, -10, 20), items, sizeStyle='small')
		#self.w.popupAction.getNSPopUpButton().setTransparent_(True)
		

		## tabs
		self.w.tabs = vanilla.Tabs((10, 26+20, -10, -14), ["LSB / RSB", "TSB / BSB", "VORG"], sizeStyle='small', callback=self.changeTabAction)
	...
	def popupResetAction(self, sender):
		...
		Glyphs.removeCallback(self.updateInterface_KOLDAVTVGJGPDKZOMU5OOPZMJI)
		###
		self.w.checkSingleMode.getNSButton().setState_(False)
		self.w.labelName.set("")
		self.w.tabs[0].button1.enable(True)
		self.w.tabs[0].button2.enable(True)
		self.w.tabs[0].button3.enable(True)
	...
	def changeTabAction(self, sender):
		...
		isSetDirection = True
		if tabIndex==0:
			self.w.resize(232, 324+20, True)
			self.w.checkSingleMode.enable(True)
			if isSingleMode:
				self.updateInterface_KOLDAVTVGJGPDKZOMU5OOPZMJI()
			if isSetDirection:
			...
			self.w.resize(232, 142+20, True)
			self.w.checkSingleMode.enable(False)
			if isSetDirection:
				setDirectionVertical()
			self.w.labelName.set("")
